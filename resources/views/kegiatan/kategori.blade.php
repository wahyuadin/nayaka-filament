...
<div class="col-lg-8">
    <h5 class="section-heading mb-4 pb-2">
        Kategori Berdasarkan: <b class="text-primary">{{ ucwords($slug ?? 'Semua Kategori') }}</b> {{-- Assumes $kategori is passed from controller --}}
    </h5>
    <div class="row">
        @foreach ($data as $dataItem)
        <div class="col-md-6 mb-4">
            <article class="entry card border-0"> {{-- Apply the .entry class for consistent styling defined in the main CSS --}}
                <div class="entry-img">
                    <img src="{{ asset('storage/' . $dataItem->image) }}" alt="{{ $dataItem->title }}" class="img-fluid">
                </div>

                <div class="card-body entry-body"> {{-- Apply .entry-body for consistent padding --}}
