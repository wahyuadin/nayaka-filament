        ...
        <div class="container" data-aos="fade-up" data-aos-delay="200">
            <div class="row g-5">
                ...
                <div class="alert alert-warning alert-dismissible fade show" role="alert">
                    <strong>Error!</strong> {{ $errors->massage }}.
                    ...
                    <div class="sidebar">
                        <div class="sidebar-item card shadow-sm mb-4">
                            <div class="card-body">
                                ...
                                <form action="{{ route('cari.kegiatan') }}" method="post" class="mt-3">
                                    @csrf
                            ...
                            <div class="card-body">
                                ...
                                <div class="mt-3">
                                    ...
                                    <div class="post-item mt-3 d-flex align-items-center">
                                        <img src="{{ asset('storage/' . $kegiatanTerbaru->image) }}" alt="Gambar recent post 1" class="img-fluid flex-shrink-0" style="width: 80px; height: 60px; object-fit: cover; border-radius: 5px;">
                                        <div class="ms-3">
                                            <P><a href="{{ route('kegiatan.slug', $kegiatanTerbaru->slug) }}">{{ $kegiatanTerbaru->title }}</a>
                                            </P>
                                            <time datetime="{{ $kegiatanTerbaru->date }}">{{ Carbon\Carbon::parse($kegiatanTerbaru->date)->locale('id')->translatedFormat('d F Y') }}</time>
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="sidebar-item card shadow-sm">
                            <div class="card-body">
                                ...
                                <div class="mt-3">
                                    ...
                                    @foreach ($item->tags->take(20) as $tag)
                                    <a href="{{ route('kegiatan.tag.slug' , $tag->slug ?? '#') }}" class="btn btn-outline-secondary btn-sm m-1" style="padding: .25rem .5rem; font-size: .75rem;">
                                        {{ $tag->title  ?? 'error'}}
                                    </a>
                                    @endforeach
                                    @endforeach
